...
static void SinoidInit(TYPE_SINOID *s,float x0,float v0,float x1,float fre,float offset);
...
void UpdateTraj(TRAJ_GEN *traj,float x0,float xd,uint32_t startTime,uint32_t duration,TRAJ_GEN_CurveTypeDef lineType)
{
	traj->repetition = 1000;
	traj->tStart = startTime;
	traj->trajState = TRAJ_GEN_STATE_BUSY;
